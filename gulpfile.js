...
gulp.task('init', cb => {
	...
	// generate themes scss
	dest = './src/scss/prism/themes/';
	fs.mkdirSync(dest,{recursive:true})
	glob.sync('./node_modules/prismjs/themes/*.css').forEach( file => {
		// fs.copyFileSync(file, dest + file.split('/').pop().split('.').shift() + '.scss' )
		let contents = '';
		let p = path.parse(file);
		contents += `/* DO NOT EDIT - Generated by gulp init */` + "\n";
		contents += `@import 'prismjs/themes/${p.name}';` + "\n";
		contents += `@import 'prismjs/plugins/line-numbers/prism-line-numbers';` + "\n";
		fs.writeFileSync( `src/scss/prism/themes/${p.name}.scss`, contents, { encoding: 'utf-8' } );
	});
	...
	require('./src/run/make-languages.js')
